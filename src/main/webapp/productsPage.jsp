    ...
    <form method="get" action="products" class="row g-3">
        ...
        <div class="col-md-3">
            <select name="sortPrice" class="form-select">
                ...
                <option value="asc" ${sortPrice == 'asc' ? 'selected' : ''}>Price: Low to High</option>
                <option value="desc" ${sortPrice == 'desc' ? 'selected' : ''}>Price: High to Low</option>
            </select>
        </div>
        <!-- Submit -->
        <div class="col-md-2">
        ...
        <c:choose>
            <c:when test="${!empty productList}">
                <c:forEach var="product" items="${productList}">
                    <div class="col-md-4">
                        <div class="card">
                            <img src="images/${product.photo}" class="card-img-top" alt="${product.name}" style="height: 200px; object-fit: cover;">
                            <div class="card-body">
                                <h5 class="card-title">${product.name}</h5>
                                <p class="card-text">$${product.price}</p>
                                <a href="cartPage.jsp" class="btn btn-success" onclick="document.getElementById('cartForm${product.id}').submit();">Add to Cart</a>
                                <a href="buyNow.jsp?productId=${product.id}&productName=${URLEncoder.encode(product.name, 'UTF-8')}&productPrice=${product.price}" class="btn btn-primary">Buy Now</a>
                                <form id="cartForm${product.id}" action="addToCart" method="post" style="display:none;">
